...
import com.amigo.baselogic.promptDateStore
import com.amigo.baselogic.userDataStore
import com.amigo.basic.BaseFragment
import com.amigo.basic.util.StatusUtils
import com.amigo.home.databinding.FragmentFeedEventBinding
import com.amigo.home.subevent.EventMyLikeFragment
import com.amigo.home.subevent.EventLikeMeFragment
...
class FeedEventFragment : BaseFragment<FragmentFeedEventBinding>(), ConversationListener {
    ...
    private fun setupTabLayout() {
        val tabData = arrayOf(
            context?.getString(com.amigo.uibase.R.string.str_message),
            context?.getString(com.amigo.uibase.R.string.str_likes),
            context?.getString(com.amigo.uibase.R.string.str_my_likes)
        )
    ...
    private fun initTabItemLayout(position: Int, title: String, tab: TabLayout.Tab) {
        ...
        if (position == 0) {
            ...
            val selectColor =
                ContextCompat.getColor(tabContext, com.amigo.uibase.R.color.color_tab_event_select)
            textView?.setTextColor(selectColor)
        }else{
            val defColor =
                ContextCompat.getColor(tabContext, com.amigo.uibase.R.color.color_tab_event_def)
...
//
//        val selectTextColor =
//            ContextCompat.getColor(tabContext, com.amigo.uibase.R.color.app_text_reverse_color)
//        val defTextColor =
//            ContextCompat.getColor(tabContext, com.amigo.uibase.R.color.app_text_color)
//
//
//        val textView = tab.customView!!.findViewById<ShapeTextView>(R.id.stv_tab)
    ...
    private fun applyNotifyPermissionDialog(context: Context) {
        ...
        val alertDialog =
            AppAlertDialog.Builder()
                .setTitle(context.getString(com.amigo.uibase.R.string.str_enable_notify))
                .setContent(context.getString(com.amigo.uibase.R.string.str_enable_notify_desc))
                .setBtnPositiveContent(context.getString(com.amigo.uibase.R.string.str_go_setting))
                .setBtnNegativeContent(context.getString(com.amigo.uibase.R.string.str_cancel))
                .create(context)
